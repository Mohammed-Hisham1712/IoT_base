...
#define HTTP_AP_CONFIG_ENDPOINT     "ap_set_info_req"
...
int http_client_post(int sock, const http_post_t* p_post)
{
    char* http_post;
    char recv_buf[512];
    uint16_t total_len;
    ...
    msg_len = snprintf(http_post, total_len, HTTP_POST_FORMAT, p_post->endpoint,
                                            p_post->host, p_post->data_len, p_post->data);
...
int main(int argc, char* argv[])
{
    ...
    if(argc > 1)
    {
        if((argv[1][0] == '-') && (argv[1][1] == '-'))
        {
            ...
            if(memcmp(opt, "ap", sizeof("ap")) == 0)
            {
                if(argc > 3)
                {
                    req_type = HTTP_REQ_AP_INFO;
                    if(process_ap_info(argv[2], argv[3], &ap_desc) < 0)
                    {
                        return -1;
                    }
                }
    ...
    switch(req_type)
    {
        case HTTP_REQ_AP_INFO:
            if(protobuf_ap_set_info_req_prepare(&ap_desc, &post.data, &post.data_len) < 0)
            {
                return -1;
            }
            post.endpoint = HTTP_AP_CONFIG_ENDPOINT;
            break;
        default:
            return -1;
    }
    ...
    if(http_client_post(sock, &post) < 0)
    {
        return -1;
    }
